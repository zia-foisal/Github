# REACH BGD Sampling Script Template
#### 22/07/2021
#### Ben Smith


# Introduction
This template is designed to give examples of the different items of code that you may need when sampling spatial data. One of the key resources that you may wish to use when doing samples is the Stratified Sampler function of the butteR packaged (developed specifically for REACH BGD and used across IMPACT). However, sampling itself is a relatively simple process that can easily be completed using basic R and some useful packages. 

Once samples have been generated it is important to check that they work correctly with the tools we use. This will typically be Maps.Me, a phone based map app that allows for offline downloads of base maps and MKL files. Key things to check are that the KML document has an appropriate document name (as this is what it will be called in Maps.Me). Note that the document name is not the same as the file name. 

Samples for camp surveys are always split by camp, with 1 file per camp. This is so that enumerators do not get confused and conduct surveys in the wrong camp by mistake. Uploading sample points to the phones does take time however and so ways to upload multiple camps at once should be explored. This may be by colouring camps different colours, or my writing multiple, non-adjacent camps within a single file. 

Samples are typically built using the structure footprints so that enumerators can go straight to shelters. When using these data sets, it is sensible to use the most up to date footprint and to remove small structures (which may be latrines) and to remove facilities (currently a points dataset). Centroids of the structure footprints for 2020 and 2019 can be found in the GIS Folders with small structures and facilities removed.

The following code chunks give examples of code that you may wish to use when writing yourown sampling scripts. Note that the different blocks may use date in different forms that may not always be compatible (e.g. spatialPolygonDatabase vs ???)

When creating samples, do not forget that some samples may be incorrect or survey recipients may not wish to take part and that it is therefore sensible to have a buffer of samples as backup. We typically use 10%, but you can discuss this with the AO.
...
butteR is a useful tool for sampling and has been used a lot by the BGD mission in the past but Zack Arno (now left) and Mehedi Khan (now Iraq mission). It can be found on GitHub. I have not successfully used it and have prefered to use by own code above. However, it is a useful tool and should be used where valuable.

The following is one example, though I have not tested it.
```{r}
library(butteR)
## Example
